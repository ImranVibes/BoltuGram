...
void processLegacy(const QString &iconPath) {
	...
	auto shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/BoltuGram Desktop.lnk";
	if (!QFile::exists(shortcut)) {
		shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/BoltuGram.lnk";
	}
	if (!QFile::exists(shortcut)) {
		return;
	}
...
void processNewShortcuts(const QString &iconPath) {
	...
	const auto shortcuts = {
		path + u"BoltuGram Desktop/BoltuGram.lnk"_q,
		path + u"BoltuGram/BoltuGram.lnk"_q,
